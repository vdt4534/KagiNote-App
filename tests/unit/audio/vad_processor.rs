...
use crate::audio::vad::{VADProcessor, VADConfig, SileroVAD, VADResult};
use crate::audio::types::{AudioData, AudioSource, VADError};
...
fn generate_silence_samples(duration_seconds: f32) -> Vec<f32> {
    ...
    for _ in 0..num_samples {
        let noise = (rand::random::<f32>() - 0.5) * 0.001; // -60dB noise floor
        samples.push(noise);
    }
